...
export function _isStale(generatedAt) {
  return Date.now() - new Date(generatedAt).getTime() > CACHE_TTL_MS;
}

export function _pairKey(prefix, me, partner) {
...
export function _titleRowToTmdbShape(row) {
  return {
    id:             row.tmdb_id,
...
export function _filterAndRank(pool, { exclude }) {
  return pool
    .filter((r) => !exclude.has(r.item.id))
    .sort((a, b) => b.score - a.score)
    .map((r) => r.item);
