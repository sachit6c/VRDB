// lib/card-stack.js
// Reusable Tinder-style card stack with 4-direction swipe.
//
// Usage:
//   const stack = mountCardStack({
//     containerEl,    // empty <div> that will own the cards
//     queue,          // array of TMDB-like items (must have .id, .poster_path, etc.)
//     onSwipe(item, dir),  // called when a card commits a swipe
//     onTap(item),         // called on a quick tap (no drag)
//     onEmpty(),           // called when the last card is gone
//   });
//   stack.push(newItem);       // append more items dynamically
//   stack.replace(newQueue);   // wholesale swap of remaining queue
//   stack.fling(dir);          // programmatic swipe of top card
//   stack.destroy();

import { posterUrl, yearOf, formatVoteCount, titleOf, genresOf } from './tmdb-client.js';

const SWIPE_THRESHOLD = 90;
const FLING_DURATION  = 280;

const DIRECTION_LABEL = {
  right: 'Watch now',
  up:    'Watch later',
  down:  'Watched',
  left:  'Hell no',
};

// Map a drag delta to a swipe direction, or null if under threshold.
// Module-scoped (pure) so it can be unit-tested in isolation.
export function decideDirection(dx, dy, threshold = SWIPE_THRESHOLD) {
  const absX = Math.abs(dx), absY = Math.abs(dy);
  if (absX < threshold && absY < threshold) return null;
  if (absX > absY) return dx > 0 ? 'right' : 'left';
  return dy > 0 ? 'down' : 'up';
}

// Card poster aspect ratio (width : height) and how much of the container the
// card is allowed to fill, plus an absolute width ceiling for large displays.
const CARD_RATIO   = 2 / 3;
const FILL_W       = 0.94;
const FILL_H       = 0.98;
const CARD_MAX_W   = 760;

export function mountCardStack({ containerEl, queue, onSwipe, onTap, onEmpty }) {
  let items = [...queue];
  let isAnimating = false;

  // Recompute the card size whenever the stack changes size — covers the
  // post-mount layout settle, viewport resizes, and switching screens. A
  // ResizeObserver fires once immediately on observe(), so this also handles
  // the initial sizing. Falls back to window resize where it's unavailable
  // (e.g. jsdom in tests).
  let ro = null;
  if (typeof ResizeObserver !== 'undefined') {
    ro = new ResizeObserver(sizeCards);
    ro.observe(containerEl);
  } else {
    window.addEventListener('resize', sizeCards);
  }
  sizeCards();
  render();

  // Size the card to the largest 2 : 3 box that fits the container, so it
  // scales up on tablets and stays snug on phones. We can't do this in pure CSS
  // here: the stack is a flex item, and container-query units (cqw/cqh) resolve
  // to 0 against a flex-sized container in some engines.
  function sizeCards() {
    const w = containerEl.clientWidth;
    const h = containerEl.clientHeight;
    if (!w || !h) return; // not laid out yet (e.g. screen hidden)
    const cardW = Math.min(w * FILL_W, h * FILL_H * CARD_RATIO, CARD_MAX_W);
    containerEl.style.setProperty('--card-w', `${Math.round(cardW)}px`);
  }

  function render() {
    containerEl.innerHTML = '';
    const visible = items.slice(0, 3);
    for (let i = visible.length - 1; i >= 0; i--) {
      const item = visible[i];
      const isTop = i === 0;
      containerEl.appendChild(buildCard(item, i, isTop));
    }
  }

  function buildCard(item, depth, isTop) {
    const card = document.createElement('article');
    card.className = 'card';
    card.dataset.tmdbId = item.id;
    card.style.zIndex = String(100 - depth);
    card.style.transform = depthTransform(depth);

    const img = posterUrl(item.poster_path, 'w780');
    const year = yearOf(item);
    const mediaLabel = item.media_type === 'tv' ? 'TV' : 'Movie';
    const rating = item.vote_average ? Number(item.vote_average).toFixed(1) : null;
    const votes = formatVoteCount(item.vote_count);
    const ratingText = rating ? ' · ★ ' + rating + (votes ? ` (${votes})` : '') : '';
    const genres = genresOf(item, 3).join(', ');

    card.innerHTML = `
      <div class="card__poster">
        ${img ? `<img src="${img}" alt="" draggable="false">` : '<div class="card__poster--empty">🎬</div>'}
        <div class="card__overlay"></div>
        <div class="card__decision" data-decision></div>
        <div class="card__info">
          <h3 class="card__title">${escapeHtml(titleOf(item))}</h3>
          <div class="card__sub">
            ${mediaLabel}${year ? ' · ' + year : ''}${ratingText}
          </div>
          ${genres ? `<div class="card__genres">${escapeHtml(genres)}</div>` : ''}
        </div>
      </div>
    `;

    if (isTop) {
      card.tabIndex = 0;
      card.setAttribute('role', 'group');
      card.setAttribute('aria-label', `${titleOf(item)} — swipe, or use arrow keys to decide and Enter for details`);
      attachGestures(card, item);
      attachKeyboard(card, item);
    }
    return card;
  }

  const KEY_TO_DIR = { ArrowRight: 'right', ArrowLeft: 'left', ArrowUp: 'up', ArrowDown: 'down' };

  function attachKeyboard(card, item) {
    card.addEventListener('keydown', (e) => {
      if (isAnimating) return;
      const dir = KEY_TO_DIR[e.key];
      if (dir) {
        e.preventDefault();
        commitSwipe(card, item, dir, { refocus: true });
      } else if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        onTap?.(item);
      }
    });
  }

  function depthTransform(depth) {
    const scale = 1 - depth * 0.04;
    const offsetY = depth * 8;
    return `translate3d(0, ${offsetY}px, 0) scale(${scale})`;
  }

  function attachGestures(card, item) {
    let startX = 0, startY = 0;
    let dx = 0, dy = 0;
    let dragging = false;
    let pointerId = null;
    let startTime = 0;
    const decisionEl = card.querySelector('[data-decision]');

    card.addEventListener('pointerdown', (e) => {
      if (isAnimating) return;
      if (e.button !== undefined && e.button !== 0) return;
      dragging = true;
      pointerId = e.pointerId;
      startX = e.clientX; startY = e.clientY;
      startTime = performance.now();
      dx = 0; dy = 0;
      card.setPointerCapture(pointerId);
      card.classList.add('is-dragging');
    });

    card.addEventListener('pointermove', (e) => {
      if (!dragging) return;
      dx = e.clientX - startX;
      dy = e.clientY - startY;
      const rot = dx / 18;
      card.style.transform = `translate3d(${dx}px, ${dy}px, 0) rotate(${rot}deg)`;
      updateDecisionHint(decisionEl, dx, dy);
    });

    const finish = () => {
      if (!dragging) return;
      dragging = false;
      try { card.releasePointerCapture(pointerId); } catch {}
      card.classList.remove('is-dragging');
      decisionEl.textContent = '';
      decisionEl.dataset.dir = '';

      const elapsed = performance.now() - startTime;
      const absX = Math.abs(dx), absY = Math.abs(dy);
      const isTap = absX < 10 && absY < 10 && elapsed < 300;

      if (isTap) {
        card.style.transform = depthTransform(0);
        onTap?.(item);
        return;
      }

      const dir = decideDirection(dx, dy);
      if (dir) {
        commitSwipe(card, item, dir);
      } else {
        card.style.transition = 'transform 0.18s ease-out';
        card.style.transform = depthTransform(0);
        setTimeout(() => { card.style.transition = ''; }, 200);
      }
    };

    card.addEventListener('pointerup', finish);
    card.addEventListener('pointercancel', finish);
  }

  function updateDecisionHint(el, dx, dy) {
    const dir = decideDirection(dx, dy, SWIPE_THRESHOLD * 0.5);
    if (!dir) { el.textContent = ''; el.dataset.dir = ''; return; }
    el.dataset.dir = dir;
    el.textContent = DIRECTION_LABEL[dir];
  }

  function commitSwipe(card, item, dir, { refocus = false } = {}) {
    if (isAnimating) return;
    isAnimating = true;
    const w = window.innerWidth + 200;
    const h = window.innerHeight + 200;
    const targets = {
      right: `translate3d(${w}px, 0, 0) rotate(20deg)`,
      left:  `translate3d(${-w}px, 0, 0) rotate(-20deg)`,
      up:    `translate3d(0, ${-h}px, 0)`,
      down:  `translate3d(0, ${h}px, 0)`,
    };
    card.style.transition = `transform ${FLING_DURATION}ms cubic-bezier(.2,.7,.3,1), opacity ${FLING_DURATION}ms`;
    card.style.transform = targets[dir];
    card.style.opacity = '0';

    // onSwipe persists the decision. It may return a promise; if it rejects we
    // restore the card so the user's swipe isn't silently lost.
    const writeResult = Promise.resolve(onSwipe?.(item, dir));
    writeResult.catch(() => {}); // pre-empt an unhandled rejection before the timer fires

    setTimeout(() => {
      items.shift();
      if (items.length === 0) {
        containerEl.innerHTML = '';
        onEmpty?.();
      } else {
        render();
        if (refocus) containerEl.querySelector('.card')?.focus();
      }
      isAnimating = false;

      // Persistence failed → put the card back at the front of the queue.
      writeResult.catch(() => {
        items.unshift(item);
        render();
        if (refocus) containerEl.querySelector('.card')?.focus();
      });
    }, FLING_DURATION);
  }

  function fling(dir) {
    const top = containerEl.querySelector('.card');
    if (!top || isAnimating || items.length === 0) return;
    commitSwipe(top, items[0], dir);
  }

  function push(item) {
    items.push(item);
    if (items.length <= 3) render();
  }

  function replace(newQueue) {
    items = [...newQueue];
    render();
  }

  function size() { return items.length; }

  // Global arrow-key swiping: the four arrows fling the top card even when no
  // card is focused, so the keyboard works the moment the screen is open. A
  // focused card handles its own keys first (and calls preventDefault), so we
  // bail on already-handled events to avoid a double swipe.
  function onGlobalKey(e) {
    if (e.defaultPrevented || isAnimating || items.length === 0) return;
    const dir = KEY_TO_DIR[e.key];
    if (!dir) return;

    const el = document.activeElement;
    if (el && (/^(INPUT|TEXTAREA|SELECT)$/.test(el.tagName) || el.isContentEditable)) return;
    // An open sheet/modal owns the keyboard — don't swipe behind it.
    if (document.querySelector('.sheet:not(.hidden), .modal:not(.hidden)')) return;
    // Only the visible, active screen's stack should respond.
    const screen = containerEl.closest('.screen');
    if (screen && !screen.classList.contains('is-active')) return;
    if (containerEl.classList.contains('hidden')) return;

    e.preventDefault();
    fling(dir);
  }
  document.addEventListener('keydown', onGlobalKey);

  function destroy() {
    window.removeEventListener('resize', sizeCards);
    document.removeEventListener('keydown', onGlobalKey);
    containerEl.innerHTML = '';
    items = [];
  }

  return { fling, push, replace, size, destroy };
}

export function escapeHtml(s) {
  return String(s)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
}
