...
export function makeDismissable(overlayEl, panelEl, onClose, scrollEl = panelEl) {
  ...
  // Begin a drag only when the scroll container is at the top, so swiping down
  // never fights with reading long content. Returns whether a drag started.
  const begin = (y) => {
    if (scrollEl.scrollTop > 0) return false; // let internal content scroll first
    dragging = true;
    startY = y;
    dy = 0;
  ...
  // Touch (mobile): drive the drag from native touch events. The panel scrolls
  // internally, so it can't use `touch-action: none` like the swipe cards do —
  // which means a plain pointer-drag gets cancelled the instant the browser
  // decides the gesture is a scroll, and the sheet never follows the finger.
  // A non-passive touchmove lets us call preventDefault() for a genuine
  // downward drag from the top, claiming the gesture before the browser does.
  ...
  panelEl.addEventListener('touchmove', (e) => {
    ...
    if (y - startY > 0 && scrollEl.scrollTop <= 0) e.preventDefault();
    move(y);
  }, { passive: false });
  ...
  // Mouse (desktop): pointer events. Skip touch pointers — handled above so we
  // don't process the same gesture twice.
  ...
  panelEl.addEventListener('pointercancel', (e) => {
    if (e.pointerType === 'touch') return;
    finish();
  });
}
